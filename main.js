(function () {
  ...
  function getTopTracks() {
    fetch('https://api.spotify.com/v1/me/top/artists?limit=5&offset=0&time_range=short_term', {
      headers: {
    ...
    .catch((error) => {
      console.error(error);
      songsPlaceholder.innerHTML = errorTemplate(error.error);
    });
  }

  //RECENT ARTISTS TEMPLATE
  function songsTemplate(data) {
    let ret = `<h1>TOP 5 ARTISTS IN THE LAST MONTH</h1>`;
    for (let i = 0; i < 5; i++) {
      ret += `<div style="display: inline-block; margin: 10px;">`;
      ret += `<h3>Artist ${i + 1}: ${data.items[i].name}</h3>`;
      ret += `<img src="${data.items[i].images[2].url}" alt="${data.items[i].name}" style="width: 220px; height: 220px;" />`;
      ret += `</div>`;
    }
    return ret;
  }
  ...
  function generateArtists() {
    ...
    .then((data) => {
      console.log(data);
      genTemplate(data).then((html) => {
        genPlaceholder.innerHTML = html;
      });
    })
    .catch((error) => {
      console.error(error);
      genPlaceholder.innerHTML = errorTemplate(error.error);
    });
  }

  async function genTemplate(data) {
    let ret = `<h1>HERE ARE SOME RECOMMENDED ARTISTS (click on the image to go to spotify)</h1>`;
    
    // Create an array of promises for fetching related artists
    const promises = data.items.map(item => newArtist(item.id));
    ...
    // Render the template using the data from related artists
    for (let i = 0; i < 5; i++) {
      const artist = relatedArtists[i].artists[Math.floor(Math.random() * 20)];
      ret += `<div style="display: inline-block; margin: 10px;">`;
      ret += `<h3>${artist.name}</h3>`;
      ret += `<a href="${artist.external_urls.spotify}" target="_blank"><img src="${artist.images[2].url}" alt="${artist.name}" class="spotify-link" style="width: 220px; height: 220px;" /></a>`;
      ret += `</div>`;
    }
    return ret;
  }

  function newArtist(id) {
...
// OLD SHI
// NEWER STUFF FOR ACTUAL PROGRAM ABOVE
  ...
  const songsPlaceholder = document.getElementById('songs');
  const genPlaceholder = document.getElementById('gen');
  ...
  document
    .getElementById('generate-artists-button')
    .addEventListener('click', generateArtists, false);

  const genButton = document.getElementById('generate-artists-button');
  genButton.addEventListener('click', () => {
      genPlaceholder.style.display = 'block'; // Or 'inline-block', depending on your needs
  });
})();
